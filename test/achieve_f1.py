...
def load_cilin(cilin_path):
    ...
    try:
        ...
        for code, words in code_to_words_temp.items():
        ...
        print(f"  - 直接同义词词条数 (word_to_direct_synonyms): {len(word_to_direct_synonyms)}")
        print(f"  - 词语编码映射数 (word_to_codes): {len(word_to_codes)}")
        print(f"  - 编码前缀种类数 (code_prefix_to_words): {len(code_prefix_to_words)}")
        return CILIN_DATA
...
def calculate_f1_with_cilin(input_excel_path,cilin_path):
    ...
    # 检查是否有足够的列 (至少需要3列来访问第二列和第三列，索引为1和2)
    if len(df.columns) < 3:
        raise ValueError("Excel文件需要至少3列才能访问第二列和第三列数据。")
    ...
    # 初始化 F1值 计算器
    print('加载F1值_词林_层次化评估，请耐心等待。。。')

    start_time = time.time()

    # 加载同义词词林
    cilin_data = load_cilin(cilin_path)
    if cilin_data is None:
        raise ValueError("词林数据加载失败，请检查词林文件路径和格式。")
    word_to_direct_synonyms, word_to_codes, code_prefix_to_words = cilin_data


    for index, question in enumerate(questions_one):
        
        # 使用jieba进行分词
        tokens1 = set(jieba.cut(question))
        tokens2 = set(jieba.cut(questions_two[index]))

        # 扩展同义词
        expanded_tokens1 = set()
        expanded_tokens2 = set()

        print(f'进行F1值_词林_层次化评估，共{questions_len}条数据，计算第{index+1}条，请耐心等待。。。')

        for token in tokens1:
            expanded_tokens1.add(token) # Add the token itself
            if token in word_to_direct_synonyms: # Add direct synonyms
                expanded_tokens1.update(word_to_direct_synonyms[token])
            if token in word_to_codes: # Add hierarchical synonyms
                for code in word_to_codes[token]:
                    prefixes_lengths = [1, 2, 4, 5] # 大类(1), 中类(2), 小类(4), 词群(5)
        ...
        for token in tokens2:
            expanded_tokens2.add(token) # Add the token itself
            if token in word_to_direct_synonyms: # Add direct synonyms
        ...
        union = expanded_tokens1.union(expanded_tokens2)

        # 计算精确率和召回率
        precision = len(intersection) / len(expanded_tokens2) if expanded_tokens2 else 0
        recall = len(intersection) / len(expanded_tokens1) if expanded_tokens1 else 0
